...
network="${1:-devnet}"
program="${2:-Bettxr9Gh1So6kvMsMhU4aRJSi1fanQT7CE1MmyZ3LDq}"
...
size_of_local_elf="$(wc -c target/deploy/fyfystream.so | cut -d' ' -f1)"

size_diff="$(( size_of_chain_elf - size_of_local_elf ))"

dd if=/dev/zero of=target/deploy/fyfystream.so bs=1 count=$size_diff \
	oflag=append conv=notrunc

chain_csum="$(sha256sum chain_bin.so | cut -d' ' -f1)"
local_csum="$(sha256sum target/deploy/fyfystream.so | cut -d' ' -f1)"
...
$local_csum  target/deploy/fyfystream.so
*****************************************************************
EOM
	exit 1
fi
